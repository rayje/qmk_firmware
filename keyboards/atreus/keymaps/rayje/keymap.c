...
#define CMDSH_4     LGUI(LSFT(KC_4))
#define CMD_L       LGUI(KC_LEFT)
#define CMD_R       LGUI(KC_RIGHT)
...
[_NU] = KEYMAP( /* Extend */
  ...
  KC_TILD, S(KC_QUOT), KC_QUOT, KC_UNDS, KC_PLUS,                   KC_MINS, KC_BSLS, KC_EQL,  S(KC_LBRC), S(KC_RBRC),
  KC_GRV,  _______,    _______, KC_MINS, _______, _______, _______, _______, _______, _______, KC_LBRC,    KC_RBRC
),
 ...
 *
 * ,----------------------------------.                     ,------------------------------------.
 * |  ESC | CTL_W|      | CTL_P|      |                     | CTL_J| CTL_L| CTL_U|   |  |CTL_MINS|
 * +------+------+------+------+------|                     +------+------+------+------+--------|
 ...
 */
[_CTL] = KEYMAP( 
  KC_ESC,  CTL_W,   _______, CTL_P,   _______,                   CTL_J,   CTL_L,   CTL_U,   KC_PIPE, CTA_MINS,
 ...
 *
 * ,------------------------------------.                     ,----------------------------------.
 * |CmdTab |CMSTab|      |CMDSH_4|      |                     | Down | Right| PgDn |      |   -  |
 * +-------+------+------+-------+------|                     +------+------+------+------+------|
 * |SCtlTab|CtlTab|      |       | PgDn |                     | Left | CMD_L| CMD_R|Insert|   "  | 
 * +-------+------+------+-------+------|                     +------+------+------+------+------|
 * |Shift  |      |      |       |      | ,------.   ,------. |  Up  | HOME | End  |      |   '  |
 * +-------+------+------+-------+------| |      |   |      | +------+------+------+------+------|
 * |       |      |      |       |      | |      |   |      | |AltSpc|      |      |      |      |
 ...
 */
[_TC] = KEYMAP(
  CMD_TAB,   ALTSH_TAB, _______, CMDSH_4, _______,                   KC_DOWN, KC_RGHT, KC_PGUP, _______, KC_MINS,
  SFCTL_TAB, LCTL_TAB,  _______, _______, KC_PGDN,                   KC_LEFT, CMD_L,   CMD_R, KC_INS,  S(KC_QUOT),
  KC_LSFT,   _______,   _______, _______, _______,                   KC_UP,   KC_HOME, KC_END,  _______, KC_QUOT,
  _______,   _______,   _______, _______, _______, _______, _______, ALT_SPC, _______, _______, _______, _______
),
